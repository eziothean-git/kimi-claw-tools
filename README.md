...
| 计费项 | 价格 | 备注 |
|--------|------|------|
| 输入 (缓存未命中) | ¥4.00/1M tokens | 首次调用 |
| 输入 (缓存命中) | ¥0.70/1M tokens | 重复prompt |
| 输出 | ¥21.00/1M tokens | 分析结果 |

### 成本估算
- **单篇论文分析**: ~¥0.04 (3K输入 + 1.5K输出)
- **每晚3篇**: ~¥0.12
- **每月(30天)**: ~¥3.6

### 预算配额 (Allegretto Plan 40%)
假设总预算为 X，则：
- **LLM API可用**: 0.4X/月
- **当前消耗**: ~¥3.6/月
- **安全边际**: 即使每日分析10篇，月成本仅¥12

**结论**: 成本极低，可放心使用LLM深度分析
